...
def main(args=None):
    if args is None:
        ...
        greater_than -- Sum files, if the total count is below a threshold, set all congruent genes to 0 (or delete)
        """)
            sys.exit(1)
        args = sys.argv[1:]
    ...
    try:
        ...
        print("Usage: /path/to/dir {zero, sum, greater_than, average}")
...
def threshold(args, which):
    ...
    if which == "greater_than":
        count_sum.write_files(count_sum.sum_greater_than(parsed_args.directory,
